...
# this is so taht we can just say "create a new board object", or "dig here", or "render this game for this object"
class Board():
    def __init__(self, dim_size, num_bombs):
        # let's keep track of these parameters, they,ll be help^ful later
    ...
    def make_new_board(self):
        ...
        while bombs_planted < self.num_bombs:
            loc = random.randint(0, self.dim_size**2 - 1) # return a random integer N such a <= N <= b
    ...
    def dig(self, row, col):
        ...
        # return True if succesfull dig, False if bomb dug
        ...
        for r in range(max(0, row-1), min(self.dim_size-1, row+1)+1):
            for c in range(max(0, col-1), min(self.dim_size-1, col+1)+1):
                if (r, c) in self.dug:
                    continue #don't dig were you've already dug
                self.dig(r,c)
...
def play(dim_size=10, num_bombs=5):
    ...
    # Step 3a: if location is a bomb, show game over messsage
    ...
    # 2 ways to end loop, lets check wich one
    if safe:
